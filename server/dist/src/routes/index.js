...
const UserDocument_1 = require("../models/UserDocument");
...
router.get("/user/:userId", validateToken_1.validateToken, async (req, res) => {
    try {
...
router.post("/user/register", async (req, res) => {
    try {
        ...
        const userExist = await User_1.User.findOne({ email: email });
        if (userExist) {
...
router.post("/document", validateToken_1.validateToken, async (req, res) => {
    try {
        const name = req.body.name;
        const text = req.body.text;
        const userId = req.user?.userId;
        const newDocument = new UserDocument_1.UserDocument({
            name,
            text: text,
            user: userId
        });
...
// GET OWNED DOCUMENTS
router.get("/documents/owned", validateToken_1.validateToken, async (req, res) => {
    try {
        const userId = req.user?.userId;
        const documents = await UserDocument_1.UserDocument.find({ user: userId, isTrashed: false });
...
router.get("/documents/shared", validateToken_1.validateToken, async (req, res) => {
    try {
        const userId = req.user?.userId;
        const sharedEdit = await UserDocument_1.UserDocument.find({ sharedEditWith: userId, isTrashed: false });
        const sharedView = await UserDocument_1.UserDocument.find({ sharedViewWith: userId, isTrashed: false });
...
router.put("/document/:id/public", validateToken_1.validateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.userId;
        const isPublic = req.body.isPublic; // client sends true or false
        const document = await UserDocument_1.UserDocument.findById(id);
        if (!document) {
...
router.get("/document/:id", validateToken_1.validateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.userId;
        const document = await UserDocument_1.UserDocument.findById(id);
...
router.delete("/document/:id", validateToken_1.validateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.userId;
        const document = await UserDocument_1.UserDocument.findById(id);
...
router.delete("/document/:id/delete", validateToken_1.validateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.userId;
        const document = await UserDocument_1.UserDocument.findById(id);
...
router.get("/documents/trash", validateToken_1.validateToken, async (req, res) => {
    try {
        const userId = req.user?.userId;
        const documents = await UserDocument_1.UserDocument.find({ user: userId, isTrashed: true });
...
router.put("/document/:id/restore", validateToken_1.validateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const document = await UserDocument_1.UserDocument.findById(id);
        if (!document) {
...
router.put("/document/:id", validateToken_1.validateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const name = req.body.name;
        const text = req.body.text;
...
router.post("/document/:id/share", validateToken_1.validateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const email = req.body.email;
        ...
        // get shared user
        const shareWithUser = await User_1.User.findOne({ email: email });
        if (!shareWithUser) {
...
router.delete("/document/:id/share/:shareUserId", validateToken_1.validateToken, async (req, res) => {
    try {
        const { id } = req.params;
...
// "LOCKS" DOCUMENT --
router.put("/document/:id/lock", validateToken_1.validateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const document = await UserDocument_1.UserDocument.findById(id);
        if (!document) {
...
// "UNLOCK" DOCUMENT --- -
router.put("/document/:id/unlock", validateToken_1.validateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const document = await UserDocument_1.UserDocument.findById(id);
        if (!document) {
...
router.get("/public/document/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const document = await UserDocument_1.UserDocument.findById(id);
        if (!document) {
